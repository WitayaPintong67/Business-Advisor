...
with cols[1]:
    if st.button("💰 Financial Analysis (Input / Revise)", use_container_width=True):
        ...
        if st.session_state.financial_inputs:
...
if st.session_state.stage == "financial":
    ...
    if st.session_state.get("financial_mode") == "revise":
        st.info("ระบบได้เปิดข้อมูล Financial Analysis เดิมขึ้นมาแล้ว ท่านสามารถแก้ไขเฉพาะรายการที่ต้องการ แล้วกด Submit เพื่อคำนวณใหม่ได้ครับ")
    else:
